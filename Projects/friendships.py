...
def add_friendship(network, name1, name2):
    ...
    # Just checking for sure that the one can not be friend with him/herself
    # This does not pose any error, the program just ignores this when
    # encountered
    if name1 == name2:
        return

    # Adding information (2-side relationship) to chosen data structure
    # 1 side
    if name1 not in network:
        network[name1] = list()
        network[name1].append(name2)
    else:
        if name2 in network[name1]:
            pass
        else:
            network[name1].append(name2)
...
def print_command(network, namelist):
    ...
    if len(namelist) != 0:
        ...
        for name in sorted(network):
            print(name)
            for value in sorted(network[name]):
                print(f"- {value}")
...
def common_friends_command(network, namelist):
    ...
    # Set default boolean value as False,
    # If there is any common friend, common will be True,
    # and then print all common friends
    # Else print the line when common is by default False,
    # which 2 names do not have common friends.
    common = False
    for a_friend in sorted(network[name1]):
        if a_friend in sorted(network[name2]):
            print(a_friend)
            common = True
